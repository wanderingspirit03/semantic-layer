...
For a workflow that must join OpenClaw bundles with another service, inject a
shared identity key as `SEMANTIC_LAYER_IDENTITY_KEY` for the first setup
process. Use a secret manager. Do not put the value in a command, config file,
or log. Setup removes the variable before it starts OpenClaw child processes
and stores the value in the owner only credential file. A rerun rejects a
...
the trusted correlation Gateway method, endpoint authentication, local spool,
the plugins that were enabled before
